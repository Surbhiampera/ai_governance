        ...
        <div style={{ flex: 1, position: "relative" }}>
          <input
            ...
            onChange={(e) => {
              ...
              // Typing something that no longer matches the selected org
              // means the user is composing a new one — deselect so the
              // create-time model picker reappears (and the edit view below
              // disappears) instead of showing both at once.
              if (selectedOrg) {
                const cur = orgs.find((o) => o.id === selectedOrg);
                const curLabel = cur
                  ? displayName(cur.org_name) || displayName(cur.id)
                  : "";
