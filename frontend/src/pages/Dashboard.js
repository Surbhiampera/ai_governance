...
function Dashboard() {
  ...
  const load = useCallback(
    async (isRefresh = false) => {
      try {
        ...
        const [ovRes, trRes, prjRes, modRes, reqRes, piiRes] =
  ...
  if (loading)
    return <div className="loading">Loading proxy governance data…</div>;
        ...
        <div className="panel">
          ...
          <div className="pill-row">
            {[
              ...
              {
                label: "Avg Latency",
                value: `${num(overview?.avg_latency_ms)} ms`,
              },
            ].map((p) => (
              <div key={p.label} className="pill">
                {p.label} <span className="highlight">{p.value}</span>
              </div>
            ))}
          </div>
          <div className="stack" style={{ marginTop: 16 }}>
            ...
            <div className="list-item">
              ...
              <div className="list-meta">
                {num(overview?.completion_tokens)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="panel">
          ...
          {byModel.length > 0 ? (
            ...
            <div className="empty-state">
              No model data yet. Send requests through the proxy.
            </div>
          )}
        </div>
              ...
              <div className="pill-row" style={{ marginBottom: 16 }}>
                <div className="pill">
                  ...
                  <span className="highlight">
                    {num(piiSummary.total_pii_requests)}
                  </span>
                </div>
                <div className="pill">
                  Blocked{" "}
                  <span className="highlight" style={{ color: "#ef4444" }}>
                    {num(piiSummary.blocked_requests)}
                  </span>
                </div>
              </div>
                  ...
                  <div className="list-grid">
                    {piiSummary.action_breakdown.map((item) => (
                      <div key={item.action} className="list-item">
                        ...
                        <div className="list-meta">
                          <span
                            className={`status-pill ${item.action === "block" ? "critical" : item.action === "mask" ? "medium" : "low"}`}
                          >
                            {item.count}
                          </span>
                        </div>
            ...
            <table>
              ...
              <tbody>
                {byProject.map((r) => {
                  const share =
                    grandTotal > 0
                      ? Math.round((r.total_cost / grandTotal) * 100)
                      : 0;
                  return (
            ...
            <tbody>
              {requests.length === 0 ? (
                <tr>
                  ...
                  >
                    No proxy requests yet. External teams need to point their AI
                    client at the proxy endpoint.
                  </td>
                </tr>
              ) : null}
              {requests.map((row) => (
