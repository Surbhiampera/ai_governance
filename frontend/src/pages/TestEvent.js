...
function ApiKeyStep({ org, project, onNext, onBack }) {
  ...
  return (
    <form onSubmit={submit}>
      <p className="panel-muted" style={{ marginBottom: 24, marginTop: 0 }}>
        Create an API key for <strong>{displayName(org.org_name) || displayName(org.id)}</strong> /{" "}
        <strong>{displayName(project.project_name) || displayName(project.id)}</strong>. The secret token
        is auto-generated — share it with the external team so their SDK can
        authenticate.
      </p>
      <div className="stack">
        <div className="field">
          <label>
            ...
            >
              — this IS the key; send it as the{" "}
              <code
                style={{
                  padding: "1px 5px",
        ...
        <div className="form-grid">
          <div className="field">
            <label>
              ...
              >
                — a label so you can identify this key
              </span>
            </label>
            <input
              value={keyName}
...
function ExistingResources({ orgs, projects, apiKeys, onRefresh }) {
  ...
  const tableHeaders = {
    org: ["Org ID", "Name", "Plan", ""],
    project: ["Project ID", "Name", "Org", ""],
    key: ["API Key ID", "Name", "Org", ""],
  };
  ...
  const sections = [
    ...
    },
    {
      ...
      setDeleting: setDeletingProject,
                ...
                {items.length === 0 ? (
                  ...
                  items.map((v) => (
                    <ResourceRow
                      ...
                      onDelete={(id) => doDelete(deleteFn, setDeleting, id)}
                    />
                  ))
                )}
              </tbody>
...
// ─── Event Injection presets + helpers ────────────────────────────────────────
...
const PRESETS = [
  {
    ...
    service_type: "email-classification",
  ...
  },
  {
    ...
    execution_type: "classify",
    latency_ms: 290,
    tags: "email-agent,classification,prospect",
    emailMeta: {
      intent: "PROSPECT_QUERY",
  ...
  },
  {
    ...
    service_type: "email-draft",
    execution_type: "draft",
    latency_ms: 1640,
    tags: "email-agent,drafting",
    emailMeta: {
      draft_generated: true,
      draft_model: "gpt-4o (Azure)",
      pipeline_status: "completed",
    },
  },
  {
    ...
    execution_type: "draft",
    latency_ms: 1920,
    tags: "email-agent,drafting,gemini",
    emailMeta: {
      draft_generated: true,
  ...
  },
  {
    ...
    execution_type: "full-pipeline",
    latency_ms: 3200,
    tags: "email-agent,pipeline,pii",
    emailMeta: {
      intent: "REFUND_REQUEST",
      intent_confidence: 0.91,
      pii_masked: true,
      masking_types: ["order_id", "tracking_id", "phone"],
      draft_generated: true,
      classification_model: "gpt-5-nano",
      draft_model: "gpt-4o (Azure)",
      pipeline_status: "completed",
  ...
  },
  {
    ...
    service_type: "email-autoreply",
    execution_type: "auto-reply",
    latency_ms: 2100,
    tags: "email-agent,autoreply,prospect",
    emailMeta: {
      intent: "PROSPECT_QUERY",
      auto_replied: true,
      draft_generated: true,
  ...
  },
  {
    ...
    service_type: "email-fetch",
    execution_type: "fetch",
    latency_ms: 620,
    tags: "email-agent,fetch,msgraph",
  ...
  },
  {
    ...
    service_type: "email-sanitization",
    execution_type: "sanitize",
    latency_ms: 480,
    tags: "email-agent,pii,high-risk",
    pii_type: "order_id",
    emailMeta: {
      pii_masked: true,
      masking_types: ["order_id", "tracking_id", "phone", "email_address"],
      pipeline_status: "completed",
    },
  },
];
...
const buildPayload = (preset, orgId, projectId) => {
  ...
  return {
    ...
    contains_pii: hasPii,
    pii_type:
      preset.pii_type || (preset.emailMeta?.masking_types || [])[0] || null,
    external_tools: preset.tools.map((t) => ({ name: t.tool_name, cost: 0 })),
    metadata_json: {
      models,
      tools: preset.tools,
      email_agent: preset.emailMeta || {},
    },
    stages: [],
    tags: (preset.tags || "")
      .split(",")
...
const groupByProject = (events) => {
  ...
  events.forEach((evt) => {
    ...
    if (!grp.latestAt || (evt.created_at || "") > grp.latestAt)
      grp.latestAt = evt.created_at;

    const metaModels = evt.metadata_json?.models;
    const sources =
      metaModels && metaModels.length > 0
        ? metaModels.map((m) => ({
            model_name: m.model_name,
            provider: m.provider || null,
            pt: Number(m.prompt_tokens || 0),
            ct: Number(m.completion_tokens || 0),
          }))
        : evt.model_name
          ? [
              {
                ...
                pt: Number(evt.prompt_tokens || 0),
                ct: Number(evt.completion_tokens || 0),
              },
            ]
          : [];
...
const TRACING_TABS = ["Onboarding", "Event Injection"];
...
function TestEvent() {
  ...
  useEffect(() => {
    loadEvents().catch(() => {});
    getLookupEventStatuses()
      .then((r) => setEventStatuses(r.data || []))
      .catch(() => {});
  }, []); // eslint-disable-line
  ...
  const runOneAuto = async () => {
    ...
    try {
      await postTelemetryEvent(
        buildPayload(preset, orgId.trim(), projectId.trim()),
      );
      setAutoCount((c) => c + 1);
      await loadEvents();
    } catch {}
  ...
  const startAuto = () => {
    if (!orgId.trim() || !projectId.trim()) {
      setMessage("Enter Organization and Project first.");
      return;
    }
    setMessage("");
    setAutoCount(0);
  ...
  const injectAll = async () => {
    ...
    for (const p of PRESETS) {
      try {
        await postTelemetryEvent(
          buildPayload(p, orgId.trim(), projectId.trim()),
        );
        count++;
    ...
    setMessage(
      `Injected ${count} events across all ${PRESETS.length} scenarios.`,
    );
    setOneLoading(false);
    await loadEvents();
  };
  ...
  const removeModelRow = (idx) =>
    setForm((f) => ({
      ...f,
      models:
        f.models.length > 1 ? f.models.filter((_, i) => i !== idx) : f.models,
    }));
  ...
  const validate = () => {
    ...
    if (!(form.models || []).some((m) => m.model_name?.trim()))
      errors.models = "At least one model required";
    if (!(form.tools || []).some((t) => t.tool_name?.trim()))
      errors.tools = "At least one tool required";
  ...
  const submitEdit = async (e) => {
    ...
    try {
      ...
      await updateTelemetryEvent(editingEventId, {
        ...
        prompt_tokens: validModels.reduce(
          (s, m) => s + (Number(m.prompt_tokens) || 0),
          0,
        ),
        completion_tokens: validModels.reduce(
  ...
  const fieldError = (n) =>
    validationErrors[n] ? { borderColor: "var(--brand-primary)" } : {};

  const activeFilterCount = [
    appliedFilter.org_id,
    appliedFilter.tool_name,
    ...
    <div className="page-shell">
      ...
      <section className="panel" style={{ padding: "18px 24px" }}>
        <div>
          ...
          >
            Onboard new tenants and simulate or inspect live telemetry across AI
            pipeline scenarios.
          </p>
        </div>
        {message && (
          <div className="feedback-msg" style={{ marginTop: 10 }}>
            {message}
          </div>
        )}
      </section>
      ...
      <div
        style={{
          display: "flex",
          borderBottom: "2px solid rgba(124,112,174,0.14)",
        }}
      >
        {TRACING_TABS.map((t) => (
          <button
            ...
            style={{
              ...
              color:
                activeTab === t ? "var(--brand-primary)" : "var(--gray-500)",
              borderBottom:
                activeTab === t
            ...
            <section className="panel" ref={formRef}>
              ...
              <form className="stack" onSubmit={submitEdit}>
                <div className="form-grid">
                  <div className="field">
                    ...
                    <input
                      ...
                      style={fieldError("org_id")}
                    />
                    {validationErrors.org_id && (
                  ...
                  <div className="field">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, latency_ms: e.target.value })
                      }
                    />
                  </div>
                  <div className="field">
                  ...
                  <div className="field">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, execution_type: e.target.value })
                      }
                    />
                  </div>
                </div>
                ...
                {validationErrors.models && (
                  <span className="field-error">{validationErrors.models}</span>
                )}
                <div
                      ...
                      <div className="form-grid">
                        ...
                        <div className="field">
                          ...
                          <input
                            value={m.provider}
                            onChange={(e) =>
                              updateModelRow(idx, { provider: e.target.value })
                            }
                        ...
                        <div className="field">
                          ...
                          <input
                            type="number"
                            min="0"
                            value={m.completion_tokens}
                            onChange={(e) =>
                              updateModelRow(idx, {
                                completion_tokens: e.target.value,
                              })
                            }
                          />
                        </div>
                ...
                {validationErrors.tools && (
                  <span className="field-error">{validationErrors.tools}</span>
                )}
                <div
                  style={{ display: "flex", flexDirection: "column", gap: 10 }}
                >
                      ...
                      <div className="form-grid">
                        <div className="field">
                          ...
                          <input
                            value={t.tool_name}
                            onChange={(e) =>
                              updateToolRow(idx, { tool_name: e.target.value })
                            }
                          />
                        </div>
                        <div className="field">
                          ...
                          <input
                            value={t.tool_type}
                            onChange={(e) =>
                              updateToolRow(idx, { tool_type: e.target.value })
                            }
                          />
                        </div>
                        <div className="field">
                          ...
                          <input
                            value={t.vendor}
                            onChange={(e) =>
                              updateToolRow(idx, { vendor: e.target.value })
                            }
                          />
                        </div>
                      </div>
                ...
                <div className="form-grid">
                  <div className="field">
                    ...
                    <input
                      value={form.pii_type}
                      onChange={(e) =>
                        setForm({ ...form, pii_type: e.target.value })
                      }
            ...
            <section className="panel">
              <div className="section-head">
                <div>
                  ...
                  >
                    Cycles through {PRESETS.length} preset scenarios every{" "}
                    {AUTO_INTERVAL_MS / 1000}s to simulate real-world AI
                    pipeline activity.
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <div style={{ flex: 1, fontSize: 13 }}>
                  {autoRunning ? (
                    ...
                    <span style={{ color: "var(--gray-500)" }}>
                      {autoCount > 0
                        ? `Stopped · ${autoCount} events injected`
                        : "Ready — set Org and Project, then start"}
                    </span>
                  )}
                </div>
              </div>
                ...
                {PRESETS.map((p, i) => (
                  <span
                    ...
                    style={{
                      ...
                      background:
                        autoRunning &&
                        presetIndexRef.current % PRESETS.length === i
                ...
                {!autoRunning ? (
                  <>
                    ...
                    >
                      {oneLoading
                        ? "Injecting…"
                        : `Inject All Once (${PRESETS.length}×)`}
                    </button>
                  </>
                ) : (
                  <button
                    ...
                    <div key={proj.project_id} className="timeline-card">
                      ...
                      >
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          >
                            {proj.events.length} event
                            {proj.events.length !== 1 ? "s" : ""}
                          </span>
                        </div>
                        <span
                          style={{
                      ...
                      >
                        <span className="metric-chip">
                          <b>{modelList.length}</b> model
                          {modelList.length !== 1 ? "s" : ""}
                        </span>
                        <span className="metric-chip">
                          $<b>{proj.totalCost.toFixed(4)}</b>
                        </span>
                        <span className="metric-chip">
                          In <b>{proj.totalPromptTokens.toLocaleString()}</b>
                        </span>
                        <span className="metric-chip">
                          Out{" "}
                          <b>{proj.totalCompletionTokens.toLocaleString()}</b>
                        </span>
                      </div>
                      ...
                      {modelList.length > 0 && (
                        ...
                        >
                          {modelList.slice(0, 4).map((m) => (
                            <span
                              key={`${m.model_name}-${m.provider}`}
                              style={{
                                fontSize: 11,
                                padding: "2px 8px",
                                borderRadius: 10,
                                background: "rgba(124,112,174,0.08)",
                                color: "var(--brand-secondary)",
            ...
            {!loadingEv &&
              ...
              (() => {
                ...
                return (
                  ...
                  >
                    {pg.length} project{pg.length !== 1 ? "s" : ""} ·{" "}
        ...
        <div className="modal-backdrop" onClick={() => setDeleteConfirm(null)}>
          ...
          >
            ...
            <p style={{ color: "var(--gray-500)", margin: "0 0 8px" }}>
              This permanently removes the event and all related records.
            </p>
            <p
              style={{
                fontWeight: 700,
            ...
            <div className="stack">
              <div className="trace-summary-bar">
                ...
                <div className="trace-summary-item">
                  ...
                  <strong style={{ fontSize: 12 }}>
                    {fmtTs(selectedTrace.event.created_at)}
                  </strong>
                </div>
              </div>
              ...
              <div className="trace-summary-bar" style={{ marginTop: 0 }}>
                <div className="trace-summary-item">
                  ...
                  <strong>
                    <span
                      className={`status-pill ${(selectedTrace.event.status || "").toLowerCase()}`}
                      style={{ fontSize: 11 }}
                    >
              ...
              <div className="trace-summary-bar" style={{ marginTop: 0 }}>
                <div className="trace-summary-item">
                  ...
                  <strong>
                    {(selectedTrace.event.prompt_tokens ?? 0).toLocaleString()}
                  </strong>
                </div>
                <div className="trace-summary-item">
                  ...
                  <strong>
                    {(
                      selectedTrace.event.completion_tokens ?? 0
                    ).toLocaleString()}
                  </strong>
                </div>
                <div className="trace-summary-item">
                  ...
                  <strong>
                    {(selectedTrace.event.total_tokens ?? 0).toLocaleString()}
                  </strong>
                </div>
                <div className="trace-summary-item">
                  <span>Latency</span>
              ...
              <div className="trace-summary-bar" style={{ marginTop: 0 }}>
                <div className="trace-summary-item">
                  ...
                  <strong>
                    ${Number(selectedTrace.event.total_cost || 0).toFixed(6)}
                  </strong>
                </div>
                <div className="trace-summary-item">
                  <span>LLM Cost</span>
                  <strong>
                    ${Number(selectedTrace.event.llm_cost || 0).toFixed(6)}
                  </strong>
                </div>
                <div className="trace-summary-item">
                  <span>Infra Cost</span>
                  <strong>
                    ${Number(selectedTrace.event.infra_cost || 0).toFixed(6)}
                  </strong>
                </div>
                <div className="trace-summary-item">
                  ...
                  <strong
                    style={{
                      color:
                        Number(selectedTrace.event.risk_score || 0) > 0.6
                          ? "var(--brand-primary)"
                          : "inherit",
                    }}
                  >
          ...
          >
            <div className="modal-header">
              <div>
                ...
                >
                  {Object.keys(selectedProject.models).length} model
                  {Object.keys(selectedProject.models).length !== 1 ? "s" : ""}{" "}
                  used
                  {" · "}
                  {selectedProject.events.length} event
                  {selectedProject.events.length !== 1 ? "s" : ""}
              ...
              <div className="trace-summary-bar">
                <div className="trace-summary-item">
                  ...
                  <strong style={{ fontSize: 20 }}>
                    {Object.keys(selectedProject.models).length}
                  </strong>
                </div>
                <div className="trace-summary-item">
                ...
                <div className="trace-summary-item">
                  <span>Prompt Tokens</span>
                  <strong>
                    {selectedProject.totalPromptTokens.toLocaleString()}
                  </strong>
                </div>
                <div className="trace-summary-item">
                  <span>Completion Tokens</span>
                  <strong>
                    {selectedProject.totalCompletionTokens.toLocaleString()}
                  </strong>
                </div>
              </div>
                  ...
                  {selectedProject.events.map((evt) => (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {fmtTs(evt.created_at)}
                        </span>
                        <span
                          className={`status-pill ${(evt.status || "").toLowerCase()}`}
                          style={{ fontSize: 10 }}
                        >
                          {evt.status}
                        </span>
