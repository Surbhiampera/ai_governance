...
const SETUP_STEPS = ["Organization", "Project", "API Key", "Done"];

function SetupStepBar({ current }) {
  return (
          ...
          <React.Fragment key={label}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
              <div style={{
                ...
                background: done || active ? "linear-gradient(135deg,#9e2a97,#7c70ae)" : "rgba(124,112,174,0.12)",
            ...
            {i < SETUP_STEPS.length - 1 && (
              <div style={{
                ...
                background: i < current ? "linear-gradient(90deg,#9e2a97,#7c70ae)" : "rgba(124,112,174,0.18)",
...
function SetupInlineError({ msg }) {
  if (!msg) return null;
  return <div className="error-message" style={{ marginTop: 12, fontSize: 14 }}>{msg}</div>;
}
    ...
    <form onSubmit={submit}>
      ...
      <div className="stack">
        <div className="form-grid">
          <div className="field"><label>Organization ID</label><input value={form.id} onChange={(e) => set("id", e.target.value)} placeholder="acme-corp" required /></div>
          <div className="field"><label>Organization Name</label><input value={form.org_name} onChange={(e) => set("org_name", e.target.value)} placeholder="Acme Corp" required /></div>
        </div>
        <div className="form-grid">
      ...
      <div className="action-row" style={{ marginTop: 24, justifyContent: "flex-end" }}>
        <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Creating…" : "Create Organization & Continue"}</button>
      </div>
    </form>
  );
}

function ProjectSetupStep({ org, onNext, onBack }) {
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div className="stack">
        <div className="form-grid">
          <div className="field"><label>Project ID</label><input value={form.id} onChange={(e) => set("id", e.target.value)} placeholder="acme-chatbot" required /></div>
          <div className="field"><label>Org ID <span style={{ opacity: 0.6 }}>(auto-filled)</span></label><input value={org.id} disabled style={{ background: "var(--gray-100)" }} /></div>
        </div>
        <div className="form-grid">
          <div className="field"><label>Project Name <span style={{ opacity: 0.6 }}>(optional)</span></label><input value={form.project_name} onChange={(e) => set("project_name", e.target.value)} placeholder="Support Chatbot" /></div>
          <div className="field"><label>Environment <span style={{ opacity: 0.6 }}>(optional)</span></label><input value={form.environment} onChange={(e) => set("environment", e.target.value)} placeholder="production" /></div>
        </div>
      </div>
      <SetupInlineError msg={error} />
      <div className="action-row" style={{ marginTop: 24, justifyContent: "space-between" }}>
        <button type="button" className="btn btn-ghost" onClick={onBack}>Back</button>
        <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Creating…" : "Create Project & Continue"}</button>
      </div>
    </form>
  );
}

function generateSetupKeyId(orgId) {
...
function ApiKeySetupStep({ org, project, onNext, onBack }) {
  const [generatedId, setGeneratedId] = useState(() => generateSetupKeyId(org.id));
  ...
  const submit = async (e) => {
    ...
    setSaving(true); setError("");
    try {
      const res = await createApiKey({ id: generatedId, org_id: org.id, project_id: project.id, key_name: keyName.trim() });
      onNext(res.data);
    } catch (err) {
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div className="stack">
        <div className="field">
          <label>Secret Token — send as <code style={{ padding: "1px 5px", background: "var(--gray-100)", borderRadius: 4, fontSize: 12 }}>X-API-Key</code> header</label>
          <div style={{ display: "flex", gap: 8 }}>
            <input value={generatedId} readOnly style={{ flex: 1, background: "var(--gray-100)", fontFamily: "monospace", fontSize: 13, color: "var(--brand-primary)", letterSpacing: "0.04em" }} />
            <SetupCopyBtn value={generatedId} />
            <button type="button" className="btn btn-ghost" onClick={() => setGeneratedId(generateSetupKeyId(org.id))} style={{ padding: "0 14px", borderRadius: 14, whiteSpace: "nowrap" }}>Regenerate</button>
          </div>
        </div>
        <div className="form-grid">
          <div className="field"><label>Key Name</label><input value={keyName} onChange={(e) => setKeyName(e.target.value)} placeholder="acme-prod" required /></div>
          <div className="field"><label>Scoped to</label><input value={`${org.id} / ${project.id}`} disabled style={{ background: "var(--gray-100)" }} /></div>
        </div>
      </div>
      <SetupInlineError msg={error} />
      <div className="action-row" style={{ marginTop: 24, justifyContent: "space-between" }}>
        <button type="button" className="btn btn-ghost" onClick={onBack}>Back</button>
        <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Creating…" : "Create API Key & Finish"}</button>
      </div>
    </form>
  );
}

function SetupDoneStep({ org, project, apiKey, onReset }) {
  return (
    <div className="stack">
      <p className="panel-muted" style={{ marginTop: 0 }}>
        Setup complete. Copy the secret token below — it cannot be retrieved again.
      </p>
      <div style={{ padding: "18px 20px", borderRadius: 14, background: "rgba(158,42,151,0.06)", border: "2px solid rgba(158,42,151,0.22)" }}>
        <span style={{ display: "block", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.14em", color: "var(--gray-500)", marginBottom: 8 }}>Secret Token (API Key ID)</span>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <code style={{ flex: 1, fontSize: 15, color: "var(--brand-primary)", fontWeight: 700, letterSpacing: "0.04em", wordBreak: "break-all" }}>{apiKey.id}</code>
          <SetupCopyBtn value={apiKey.id} />
        </div>
      </div>
      {[{ label: "Organization ID", hint: "pass as org_id", value: org.id }, { label: "Project ID", hint: "pass as project_id", value: project.id }, { label: "Key Name", hint: "display label", value: apiKey.key_name || "—" }].map(({ label, hint, value }) => (
        <div key={label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderRadius: 14, background: "var(--gray-50)", border: "1px solid rgba(124,112,174,0.16)" }}>
          <div>
            <span style={{ display: "block", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.14em", color: "var(--gray-500)", marginBottom: 3 }}>{label}</span>
          ...
          {value !== "—" && <SetupCopyBtn value={value} />}
        </div>
      ))}
      <div style={{ padding: "14px 16px", borderRadius: 14, background: "rgba(34,139,98,0.07)", border: "1px solid rgba(34,139,98,0.18)", color: "#228b62", fontSize: 14 }}>
        All three resources are live. Share the secret token with the external team.
      </div>
      <div className="action-row" style={{ justifyContent: "flex-end" }}>
        <button type="button" className="btn btn-secondary" onClick={onReset}>Set up another tenant</button>
      </div>
    </div>
  );
}

function TenantResourceRow({ label, value, onDelete, deleting }) {
  return (
    <tr>
      <td><code style={{ color: "var(--brand-primary)", fontSize: 13 }}>{value.id}</code></td>
      <td>{label === "org" ? value.org_name : label === "project" ? (value.project_name || "—") : (value.key_name || "—")}</td>
      <td>{label === "project" && <code style={{ fontSize: 12 }}>{value.org_id}</code>}{label === "key" && <code style={{ fontSize: 12 }}>{value.org_id || "—"}</code>}{label === "org" && (value.plan_type || "—")}</td>
      <td>
        <button className="btn btn-ghost" style={{ padding: "5px 12px", fontSize: 12, borderRadius: 10, color: "var(--brand-primary)" }} onClick={() => onDelete(value.id)} disabled={deleting}>
          {deleting ? "…" : "Delete"}
        </button>
      </td>
    </tr>
...
function TenantExistingResources({ orgs, projects, apiKeys, onRefresh }) {
  ...
  const doDelete = async (fn, setDeleting, id) => {
    setDeleting(id);
    try { await fn(id); await onRefresh(); } finally { setDeleting(null); }
  };

  const sections = [
    { label: "org",     items: orgs,    deleting: deletingOrg,     setDeleting: setDeletingOrg,     deleteFn: deleteOrganization, headers: ["Org ID", "Name", "Plan", ""],    empty: "No organizations yet." },
    { label: "project", items: projects, deleting: deletingProject, setDeleting: setDeletingProject, deleteFn: deleteProject,      headers: ["Project ID", "Name", "Org", ""], empty: "No projects yet." },
    { label: "key",     items: apiKeys, deleting: deletingKey,     setDeleting: setDeletingKey,     deleteFn: deleteApiKey,       headers: ["API Key ID", "Name", "Org", ""], empty: "No API keys yet." },
  ];

  return (
    <div className="stack" style={{ marginTop: 40 }}>
      <h3 style={{ margin: 0, color: "var(--gray-700)" }}>Registered Tenants</h3>
      {sections.map(({ label, items, deleting, setDeleting, deleteFn, headers, empty }) => (
        <div key={label} className="table-wrap">
          <table>
            <thead><tr>{headers.map((h, i) => <th key={i}>{h}</th>)}</tr></thead>
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={4} style={{ color: "var(--gray-500)", fontStyle: "italic" }}>{empty}</td></tr>
              ) : items.map((v) => (
                <TenantResourceRow key={v.id} label={label} value={v} deleting={deleting === v.id} onDelete={(id) => doDelete(deleteFn, setDeleting, id)} />
              ))}
            </tbody>
          </table>
        </div>
...
function TenantOnboardingPanel() {
  ...
  const loadAll = useCallback(async () => {
    setLoadError("");
    try {
      const [o, p, k] = await Promise.all([getOrganizations(), getProjects(), getApiKeys()]);
      setOrgs(o.data || []);
      setProjects(p.data || []);
      setApiKeys(k.data || []);
    } catch { setLoadError("Could not load existing resources."); }
  }, []);

  useEffect(() => { loadAll(); }, [loadAll]);

  const reset = () => { setStep(0); setOrg(null); setProject(null); setApiKey(null); loadAll(); };

  return (
    <>
      <div className="hero-card" style={{ borderRadius: "var(--radius-lg)", padding: 28 }}>
        <p>Register an external team's organization, project, and API key so their AI tools can start sending telemetry to this governance server.</p>
      </div>

      <div className="panel">
        <SetupStepBar current={step} />
        {step === 0 && <OrgSetupStep onNext={(data) => { setOrg(data); setStep(1); }} />}
        {step === 1 && org && <ProjectSetupStep org={org} onNext={(data) => { setProject(data); setStep(2); }} onBack={() => setStep(0)} />}
        {step === 2 && org && project && <ApiKeySetupStep org={org} project={project} onNext={(data) => { setApiKey(data); setStep(3); loadAll(); }} onBack={() => setStep(1)} />}
        {step === 3 && org && project && apiKey && <SetupDoneStep org={org} project={project} apiKey={apiKey} onReset={reset} />}
      </div>

      {loadError && <div className="error-message">{loadError}</div>}
      <TenantExistingResources orgs={orgs} projects={projects} apiKeys={apiKeys} onRefresh={loadAll} />
    </>
  );
}
...
function AlertsSecurity() {
  ...
  return (
    <div className="page-shell">
      {/* ── Page Header + Tab Bar ── */}
      <div className="panel" style={{ padding: "20px 24px 0" }}>
          ...
          {SECTIONS.map((s) => (
            <button
              ...
              style={{
                ...
                color: activeSection === s.id ? "var(--brand-primary)" : "var(--gray-500)",
                borderBottom: activeSection === s.id ? "2px solid var(--brand-primary)" : "2px solid transparent",
                marginBottom: -2,
                fontSize: 14,
      ...
      {/* ── Tenant Setup Tab Content ── */}
      {activeSection === "tenant-setup" && <TenantOnboardingPanel />}
